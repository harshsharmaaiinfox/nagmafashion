...
import { CategoryOption } from '../../../shared/data/categoryOption';

function flattenCategoryOption(cats: any[]): any[] {
  const result: any[] = [];
  cats.forEach(cat => {
    result.push(cat);
    if (cat.subcategories?.length) result.push(...flattenCategoryOption(cat.subcategories));
  });
  return result;
}

const FLAT_CATEGORY_OPTIONS: any[] = flattenCategoryOption(CategoryOption);
...
export class ParisComponent {
  ...
  public readonly flatCategoryOptions: any[] = FLAT_CATEGORY_OPTIONS;
